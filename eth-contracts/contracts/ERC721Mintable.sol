...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "Only the contract owner is allowed to execute this action!");
        _;
    }

    //  4) fill out the transferOwnership function
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "Cannot assign the ownership to the zero address!");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(owner != to, "The given address must not be the owner tof the token!");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isOwner(msg.sender) || isApprovedForAll(owner, msg.sender), "You are not allowed to approve the given token!");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), "A token with the specified id already exists!");
        require(to != address(0), "Cannot mint a token to the zero address!");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        address owner = ownerOf(tokenId);
        require(owner == from, "Not the correct owner of the specified token");

        // require token is being transfered to valid address
        require(to != address(0), "Cannot transfer a token to the zero address!");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), "Token with the specified id does not exist!");
...
//      - make the base token uri: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/
    constructor(string memory symbol, string memory name) public ERC721Metadata(
        symbol, name, 
...
//          => tokenURI as parameter does not make sense, as it is calculated from the tokenId
